...
- `1.17.0`：為單日與3日規劃器加入顯示事件數量的可收合全天區域，並以清晰的緊湊卡片、時間提示與防重疊欄位改善20分鐘以內的事件。
...
- **單日規劃器**：在24小時時間軸上規劃一天。全天區域會顯示事件數量、支援收合並記住狀態。多日範圍顯示為連續的全天橫條或日期時間區間，帶時間範圍的邊界可跨日期調整。20分鐘以內的事件使用帶時間提示與防重疊欄位的清晰緊湊卡片。
...
- **行動裝置最佳化**：獨立控制項使用44/48 px觸控目標；對話框操作會依螢幕寬度重新排列；尺寸會兼顧安全區域與軟體鍵盤。月度格狀檢視也支援雙指縮放、重設縮放和當日摘要面板。
