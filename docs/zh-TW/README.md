...
- `1.15.0`：範圍跨越週、月或年份邊界時會再次顯示標題。檔案選項現在可將單日筆記轉換為範圍筆記，並直接編輯資料夾和包含日期的完整檔名。
- `1.14.0`：調整行動裝置 modal 操作按鈕的尺寸和排列，移除多餘的容器樣式，並避免按鈕被軟體鍵盤遮住。
- `1.13.3`：調整所有支援語言的介面文字。
- `1.13.0`：改善窄螢幕下的單日規劃版面和 modal 操作區。
- `1.12.1`：更新含有已知漏洞的 parser 相依套件。
- `1.12.0`：在設定中加入意見回饋、錯誤回報和功能需求連結。
- `1.11.0`：新增多日檢視逐日導覽、單日與月度列表檢視的剪貼簿操作，並提升跨欄調整範圍邊界的穩定性。
- `1.10.2`：避免拖曳結束後誤觸時間軸。
...
- [提供意見](https://github.com/POBSIZ/obsidian-diary/issues/new?template=feedback.yml)
