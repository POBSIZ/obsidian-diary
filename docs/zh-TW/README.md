...
- **單日規劃器**：在24小時時間軸上規劃一天。帶 `start_time` 和 `end_time` 的筆記顯示在時間軸；全天與無時間筆記分開顯示。
...
| `start_time` | 選用的 chip 開始時間，格式為 `HH:mm`。有時間的 chip 會依開始時間排序。 |
| `end_time` | 選用的 chip 結束時間，格式為 `HH:mm`。此值不會觸發提醒。 |
