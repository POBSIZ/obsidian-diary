...
Diary 是一個 Obsidian 社群外掛，可以把 vault 中的 Markdown 檔案呈現為依日期組織的規劃視圖。你可以在年度總覽、月度格狀檢視和月度列表之間切換，同時管理單日筆記、範圍筆記、月度和年度計畫筆記、節假日、一個日曆疊加標籤、可選的外部日曆、待辦狀態和本機提醒。
...
以下截圖來自一個含有範例規劃筆記的全新 demo vault。行動裝置截圖是在啟用外掛行動裝置渲染路徑後拍攝。右側邊欄規劃器使用同一個月度格狀檢視的緊湊模式，因此完整月度格狀截圖仍是主要視覺參考。
...
- **重複事件**：依每日、每月或每年重複筆記，可選公曆或替代日曆作為基準。Diary 只會建立目前可見規劃範圍內的發生項。
...
- 啟用 **Repeat** 後，選擇每日、每月或每年，以及日曆基準。發生項會依目前可見年份或月份延遲建立；Diary 只更新同一 `recurrence_id` 系列檔案，並跳過無關的既有筆記。
...
- 在月度格狀中使用雙指縮放。
- 使用重設縮放按鈕恢復格狀縮放等級。
- 使用 **行動裝置底部間距** 和 **行動裝置儲存格寬度** 調整間距和儲存格寬度。
...
| 行動裝置底部間距 | 行動裝置規劃器底部間距，避免內容被 Obsidian 行動裝置控制項遮擋。 |
...
| `recurrence_rule` | 簡單頻率規則：`FREQ=DAILY`、`FREQ=MONTHLY` 或 `FREQ=YEARLY`。 |
...
重複發生項生成是冪等的。如果目標檔案已屬於同一 `recurrence_id`，Diary 會刷新其系列 metadata；如果該路徑是一般筆記或另一系列，Diary 會保持不變。
...
- 本 repo 會將 release assets 作為個別檔案發布，並由 release workflow 為 `main.js`、`manifest.json`、`styles.css` 產生 build provenance attestation。
