...
Diary は、Obsidian の保管庫（vault）にある通常の Markdown ノートをカレンダーやタイムラインで表示するコミュニティプラグインです。専用データベースに移すことなく、年間・月間・日別・3日表示を切り替えられます。
...
- `1.17.0`: 日別・3日プランナーの終日セクションに件数表示と折りたたみ機能を追加し、20分未満の予定を読みやすいコンパクトカード、時刻ツールチップ、重なりを避ける列配置で改善しました。

過去の変更内容は [Releases](https://github.com/POBSIZ/obsidian-diary/releases) を参照してください。

## スクリーンショット

スクリーンショットには、終日・時刻付き・期間・タスク・計画ノートを入れた一時的なデモフォルダを使いました。狭幅の例は、同じ画面を限られた横幅で表示したものです。撮影後、デモフォルダは削除しました。
...
- **年間プランナー**: 日付ノートと期間ノートを `12 か月 × 31 日` の表で確認できます。広げた月セルの幅は再読み込み後も保持されます。
- **ローカライズ UI**: Diary を英語、ドイツ語、スペイン語、フランス語、日本語、簡体字中国語、繁体字中国語、韓国語に切り替えられます。
- **月間グリッド**: 1 か月を大きなカレンダーで表示し、チップ、期間バー、祝日、カレンダーオーバーレイ、外部カレンダーを確認できます。
- **月間リスト**: 月の予定を日ごとに並べ、`All`、`With notes`、`Upcoming` で絞り込めます。
- **日別プランナー**: 1日を24時間のタイムラインで計画します。終日セクションは件数を表示し、折りたたみ状態を記憶します。複数日の範囲は連続する終日バーまたは日時区間として表示され、時刻付き範囲の境界は日付をまたいで調整できます。20分未満の予定は、時刻ツールチップと重なりを避ける列を備えた読みやすいコンパクトカードで表示されます。
...
- **日付/期間ノート**: 単日または期間のファイル名をもとにノートをチップとして表示します。既定では 保管庫全体をスキャンし、設定でプランナーフォルダ内に限定できます。
- **繰り返しイベント**: グレゴリオ暦または代替カレンダーを基準に、N日・週・か月・年ごとに繰り返せます。発生分は選択するまで仮想表示されます。
- **色、タスク、完了状態**: フロントマターの `color`、`todo`、`completed` がチップの見た目とラベルに反映されます。
- **祝日オーバーレイ**: 対応 UI 言語圏の国別祝日を表示し、祝日バッジを選択すると名前を確認できます。
...
- **外部カレンダーオーバーレイ**: `webcal://` または `https://` の `.ics` フィードを追加し、手動または間隔で更新できます。イベントは読み取り専用チップ/期間として表示され、必要なものだけ Markdown ノートにできます。
...
- **プランナークリップボード**: デスクトップでは選択した日付やチップをコピー、貼り付け、削除し、貼り付けを取り消せます。
- **キーボードとアクセシビリティ**: 日付セル、チップ、期間バー、祝日バッジ、プランナーラベル、月間リスト行はキーボード操作とアクセシブルなラベルに対応しています。
- **モバイル最適化**: 単独コントロールは 44/48 px のタッチ領域を使い、モーダルアクションは画面幅に応じて再配置されます。モーダルはセーフエリアとソフトウェアキーボードを考慮し、月間グリッドのピンチズーム、ズームリセット、日別サマリーシートにも対応します。
...
5. **作成** を選択します。Diary は Markdown ノートを作成し、チップまたは期間バーとして表示します。

作成されるノートは通常の Markdown ファイルです。プラグインを無効にしても 保管庫に残ります。
...
- フィード URL はローカルのプラグインデータに保存され、保管庫と同期される可能性があります。非公開の iCal URL はアクセストークンとして扱ってください。
...
- 外部イベントは読み取り専用で、薄い点線で区別されます。ドラッグ、プランナークリップボード、タスク、色編集はできません。
- 外部イベントを選択すると、**Create Markdown note**、**Refresh calendar**、**Copy details** を持つ詳細モーダルが開き、押下・読み込み・完了のフィードバックも表示されます。
- 外部イベントから Markdown ノートを作成すると、Diary はリンク用フロントマターを保存し、そのイベントの重複オーバーレイを隠します。
...
- サフィックスを付けるとチップタイトルになります。例: `2026-05-19-mobile-qa.md` -> `mobile-qa`
- 色を設定するとチップの枠線またはモバイルの点として表示されます。
- **Todo file** を有効にするとチップにタスク状態が表示されます。
...
- 年間プランナーでは縦バーと開始日のチップとして表示されます。月間グリッドとリストでは期間バーとして表示されます。
...
プランナーのチップまたは期間バーを選択すると、ファイルオプションモーダルが開きます。

- **Single date** と **Range** を切り替える
- 既存フォルダーを選ぶか、任意のフォルダーパスを入力する
- 日付または期間と、タイトルに使う接尾辞を含むファイル名全体を編集する
- 開始日と終了日を変更する。ファイル名も自動で同期される
- チップの色を変更
...
変更を適用すると Markdown ファイルが移動または名前変更されます。期間へ変換すると `date_start` と `date_end` が書き込まれ、単日に戻すと削除されます。移動先に同名ファイルがある場合は変更できません。

デスクトップでは、日付チップや期間バーを別の日へドラッグして移動できます。期間ノートは開始日を基準に移動し、期間の長さは保たれます。移動先のパスが存在する場合、Diary はファイルを移動しません。

## キーボードとアクセシビリティ

- フォーカスされた日付セル、チップ、期間バー、祝日バッジ、月間リスト行、年ラベル、月ラベルで `Enter` または `Space` を押すと実行できます。
...
プランナーで日付やチップを選択するとき、macOS では `Cmd`、Windows/Linux では `Ctrl` を押します。
...
| Planner note scan scope | Diary が保管庫全体を探すか、**Planner folder** とその下位フォルダだけを探すかを決めます。既定: `Entire vault`。 |
...
| フィードバックとサポート | フィードバック、バグ報告、機能提案用の公開 GitHub フォームを開きます。 |
...
| キー | 説明 |
| --- | --- |
| `color` | チップの色。有効な CSS 色を使えます。例: `#22c55e`, `red`, `rgb(34, 197, 94)` |
| `todo` | `true` のときタスクのチップとして表示します。 |
...
Diary は既定で 保管庫全体の Markdown ファイルをスキャンし、次のルールに合うファイルを表示します。設定で **Planner folder** とその下位フォルダに限定できます。新しいノートは既定で **Planner folder** に作成されます。
...
- [フィードバックを送る](https://github.com/POBSIZ/obsidian-diary/issues/new?template=フィードback.yml)
- [バグを報告](https://github.com/POBSIZ/obsidian-diary/issues/new?template=bug_report.yml)
- [機能を提案](https://github.com/POBSIZ/obsidian-diary/issues/new?template=feature_request.yml)

これらのリンクは公開 GitHub Issue を開くため、GitHub アカウントが必要です。非公開の 保管庫の内容、カレンダー URL、アクセストークン、その他の機密情報を含めないでください。

## プライバシーとネットワーク

- プランナー機能は 保管庫内のローカル Markdown ファイルで動作します。
- Diary に隠れた telemetry や analytics はありません。
- 祝日とカレンダーオーバーレイの計算には、同梱データ、ブラウザ提供データ、またはローカル保存プロファイルを使います。保管庫の内容は外部サービスへ送りません。
- 外部カレンダーフィードは任意です。Diary は追加したフィード URL だけを取得し、イベントキャッシュをプラグインデータに保存します。**Create Markdown note** を選ばない限り Markdown ファイルは作成しません。
- ローカルリマインダーは Obsidian 内で処理され、ネットワークへデータを送信しません。
...
- チップが表示されない場合は、ファイル名が `YYYY-MM-DD` または `YYYY-MM-DD--YYYY-MM-DD` に従っているか確認してください。
