...
- `1.15.0`: 週・月・年の境界をまたいでも期間タイトルが表示されます。ファイルオプションでは、単日ノートを期間ノートへ変換し、フォルダーと日付を含むファイル名全体を直接編集できます。
- `1.14.0`: モバイルのモーダル操作ボタンの大きさと配置を整え、余分なコンテナ装飾を外し、ソフトウェアキーボードでボタンが隠れないようにしました。
- `1.13.3`: すべての対応言語で UI 文言を見直しました。
- `1.13.0`: 狭い画面での日別コンパクト表示とモーダル操作を改善しました。
- `1.12.1`: 既知の脆弱性があるパーサー依存関係を更新しました。
- `1.12.0`: フィードバック、バグ報告、機能要望へのリンクを設定に追加しました。
- `1.11.0`: 複数日ビューの日単位移動、日別・月間リストのクリップボード操作、列をまたぐ期間境界調整の安定化を追加しました。
- `1.10.2`: ドラッグ操作後にタイムラインのクリックが発生しないようにしました。
...
- [フィードバックを送る](https://github.com/POBSIZ/obsidian-diary/issues/new?template=feedback.yml)
...
これらのリンクは公開 GitHub Issue を開くため、GitHub アカウントが必要です。非公開の Vault 内容、カレンダー URL、アクセストークン、その他の機密情報を含めないでください。
