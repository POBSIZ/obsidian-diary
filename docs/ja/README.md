...
- **日別プランナー**: 1日を24時間のタイムラインで計画します。`start_time` と `end_time` のあるノートは時間軸に、終日・時刻未指定ノートは別領域に表示されます。
...
| `start_time` | 任意の chip 開始時刻（`HH:mm`）。時刻付き chip は開始時刻順に並びます。 |
| `end_time` | 任意の chip 終了時刻（`HH:mm`）。この値だけではリマインダーは実行されません。 |
