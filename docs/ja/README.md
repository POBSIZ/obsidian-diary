...
Diary は、Obsidian の vault 内にある Markdown ファイルを日付ベースのプランナーとして表示するコミュニティプラグインです。年表示、月間グリッド、月間リストを切り替えながら、単日ノート、期間ノート、月次・年次の計画ノート、祝日、カレンダーオーバーレイ、任意の外部カレンダー、todo 状態、ローカル通知をまとめて扱えます。
...
- **繰り返しイベント**: ノートを毎日、毎月、毎年で繰り返し、グレゴリオ暦または代替カレンダーを基準にできます。Diary は表示中の範囲に入る発生分だけを作成します。
...
- **Repeat** を有効にし、毎日・毎月・毎年とカレンダー基準を選びます。発生分は表示中の年または月に対して遅延生成され、Diary は同じ `recurrence_id` のファイルだけを更新し、無関係な既存ノートは変更しません。
...
- **モバイル下部余白** と **モバイルセル幅** で間隔とセル幅を調整できます。
...
| モバイル下部余白 | モバイルの Obsidian 操作領域に隠れないようにする下部余白。 |
...
| `recurrence_rule` | 単純な頻度ルール: `FREQ=DAILY`, `FREQ=MONTHLY`, `FREQ=YEARLY`。 |
...
繰り返し発生分の生成は冪等です。対象ファイルが同じ `recurrence_id` に属していればシリーズ metadata を更新します。通常ノートまたは別シリーズであれば変更しません。
...
- モバイルで下部が隠れる場合は、**モバイル下部余白** を増やしてください。
