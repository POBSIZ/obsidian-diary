...
현재 배포 버전은 일반 `YYYY-MM-DD` 파일명을 기준으로 동작합니다. 커스텀 판타지 캘린더는 표시/입력 미리보기 중심의 1차 기능으로 들어갔고, 아래 항목은 기존 Markdown 파일 모델을 유지하면서 확장하기 위한 후속 구현 계획입니다.
...
- **외부 캘린더 오버레이**: `webcal://` 또는 `.ics` 일정을 읽기 전용 chip/range overlay로 보여주고, 사용자가 선택한 일정만 Markdown 노트로 만듭니다. 자세한 계획은 [외부 캘린더 오버레이 구현 계획서](external-calendar-overlay-implementation-plan.md)를 참고하세요.
