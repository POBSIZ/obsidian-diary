...
- `1.15.0`: 기간이 주·월·연도 경계를 넘어도 제목이 다시 표시됩니다. 파일 옵션에서 단일 날짜 노트를 기간 노트로 바꾸고, 폴더와 날짜가 포함된 전체 파일 이름을 직접 수정할 수 있습니다.
- `1.14.0`: 모바일 모달의 액션 버튼 크기와 배치를 정리하고, 불필요한 액션 컨테이너 스타일을 제거했으며, 소프트웨어 키보드가 버튼을 가리지 않도록 개선했습니다.
- `1.13.3`: 모든 지원 언어의 UI 문구를 자연스럽게 다듬었습니다.
- `1.13.0`: 좁은 화면의 일별 플래너와 모달 액션 배치를 개선했습니다.
- `1.12.1`: 알려진 취약점이 있는 parser 의존성을 업데이트했습니다.
- `1.12.0`: 설정에 피드백, 버그 신고, 기능 제안 링크를 추가했습니다.
- `1.11.0`: 여러 날 뷰의 하루 단위 이동, 일별·월간 목록의 클립보드 작업, 날짜 열을 넘는 기간 경계 조절 안정화를 추가했습니다.
- `1.10.2`: 드래그를 마친 뒤 타임라인 클릭이 함께 실행되지 않도록 수정했습니다.
