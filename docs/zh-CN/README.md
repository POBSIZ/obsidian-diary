...
- `1.15.0`：范围跨越周、月或年份边界时会再次显示标题。文件选项现在可以把单日笔记转换为范围笔记，并直接编辑文件夹和包含日期的完整文件名。
- `1.14.0`：调整移动端模态框操作按钮的尺寸和布局，移除多余的容器样式，并避免按钮被软键盘遮挡。
- `1.13.3`：优化所有支持语言的界面文案。
- `1.13.0`：改善窄屏下的单日计划布局和模态框操作区。
- `1.12.1`：更新存在已知漏洞的解析器依赖。
- `1.12.0`：在设置中加入反馈、错误报告和功能请求链接。
- `1.11.0`：新增多日视图逐日导航、单日和月度列表视图的剪贴板操作，并提高跨列调整范围边界的稳定性。
- `1.10.2`：避免拖动结束后误触时间轴。
- `1.10.1`：将单日范围布局中的较新数组 helper 替换为兼容 ES2020 的类型安全逻辑，消除 `no-unsafe-assignment` 和 `no-unsafe-call` 审查警告，并从发布源码中排除本地代理说明。
- `1.10.0`：在单日和3日计划器中将多日范围显示为连续全天条或日期时间区间，并支持跨日期选择和边界调整。
- `1.9.4`：移除可选的产物证明；这些证明虽通过 GitHub 加密验证，却被 Obsidian Community Scorecard 拒绝。
- `1.9.3`：使用当前的构建来源操作和轻量 Git 标签，使发布来源符合 Obsidian Community Scorecard 的验证方式。
- `1.9.2`：在 JavaScript 和 CSS 发布资源中写入插件版本，使每个版本都具有唯一摘要和明确的来源证明。
- `1.9.1`：使用当前的证明操作，为每个发布资源分别生成可由 GitHub 验证的来源证明。
- `1.9.0`：让移动端计划内容、时间轴末尾和紧凑视图菜单始终位于 Obsidian 标准或浮动底部导航之上，同时保留可配置的最小间距。
...
- [提交反馈](https://github.com/POBSIZ/obsidian-diary/issues/new?template=feedback.yml)
