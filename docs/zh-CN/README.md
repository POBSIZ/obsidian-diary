...
Diary 是一款 Obsidian 社区插件，可把仓库中的普通 Markdown 笔记显示为日历和时间轴。无需迁移到专用数据库或文件格式，即可在年度、月度、单日和3日视图之间切换。
...
- `1.17.0`：为单日和3日计划器添加带事件数量的可折叠全天区域，并通过清晰的紧凑卡片、时间提示和防重叠列布局改善20分钟以内的事件。

更早的变更请参阅 [Releases](https://github.com/POBSIZ/obsidian-diary/releases)。

## 截图

截图使用了一个临时演示文件夹，其中包含全天、定时、范围、待办和计划笔记。窄屏示例展示了同一界面在横向空间不足时的效果；拍摄后已删除演示文件夹。
...
- **年度计划器**：用 `12 个月 × 31 天` 的表格查看日期笔记和范围笔记。调整后的月份单元格宽度会在重新加载后保留。
- **本地化界面**：Diary 可在英语、德语、西班牙语、法语、日语、简体中文、繁体中文和韩语之间切换。
- **月度网格**：用大型日历网格查看一个月中的条目、范围条、节假日、日历叠加标签和外部日历叠加。
- **月度列表**：按日期浏览整月计划，并用 `All`、`With notes`、`Upcoming` 筛选。
- **单日计划器**：在24小时时间轴上规划一天。全天区域会显示事件数量、支持折叠并记住状态。多日范围显示为连续的全天条或日期时间区间，带时间范围的边界可跨日期调整。20分钟以内的事件使用带时间提示和防重叠列布局的清晰紧凑卡片。
...
- **日期和范围笔记**：根据单日和日期范围文件名，在计划器中显示对应条目。默认扫描整个仓库，也可以设置为只扫描计划文件夹。
- **重复事件**：以公历或备用日历为基准，每N天、周、月或年重复。发生项在选中前保持为虚拟日程。
- **颜色、待办和完成状态**：frontmatter 中的 `color`、`todo`、`completed` 会反映在条目样式和标签上。
...
- **外部日历叠加**：添加 `webcal://` 或 `https://` `.ics` 订阅源，手动或定时刷新，并以只读条目/范围条显示在各计划视图中。只有在你选择外部事件时，才会创建普通 Markdown 笔记。
- **限定样式作用域**：Diary 的 CSS 只作用于计划视图、设置面板和插件模态框，不会改变普通仓库内容样式。
- **本地提醒**：带有 `notify_minutes` 的笔记会在事件日期、Obsidian 打开时显示 Obsidian Notice。
- **计划器剪贴板**：桌面端可以复制、粘贴、删除和撤销粘贴选中的日期或条目。
- **键盘和可访问性**：日期单元格、条目、范围条、节假日徽标、计划器标签和月度列表行都支持键盘激活和可访问标签。
- **移动端优化**：独立控件使用44/48 px触控目标；模态框操作会随屏幕宽度重新排列；尺寸会兼顾安全区域和软键盘。月度网格还支持双指缩放、重置缩放和当日摘要面板。
...
5. 选择 **创建**。Diary 会创建 Markdown 笔记，并在计划器中显示为条目或范围条。

创建出的笔记都是普通 Markdown 文件。即使禁用插件，它们也会留在仓库中。
...
- 订阅源 URL 保存在本地插件数据中，并可能随仓库同步。请把秘密 iCal URL 当作访问令牌处理。
- Diary 会拒绝本地或私有网络日历 URL。
- 你可以设置订阅源名称、颜色、是否包含描述、刷新间隔，以及在哪些计划界面显示。
- 新订阅源默认每 60 分钟自动刷新，并显示在年度、月度网格、月度列表和侧边栏视图中。
- 外部事件是只读计划叠加，以浅色虚线区分，不支持拖拽、计划器剪贴板、待办或颜色编辑。
...
- 添加后缀可作为条目标题。例如：`2026-05-19-mobile-qa.md` -> `mobile-qa`
- 设置颜色会显示条目边框或移动端圆点。
- 启用 **Todo file** 可在条目上显示待办状态。
...
- 年度计划器使用竖向条和开始日期条目显示范围笔记。月度网格和列表使用范围条显示。
- 选择 **全天** 可将两个时间都留空；如需指定时间，请同时填写两者，以定义从 `date_start` + `start_time` 到 `date_end` + `end_time` 的连续区间。
- 单日和3日计划器将全天范围显示为跨日期的连续条。可将其拖到时间轴上分配时间，再拖动首日或末日边界来同时调整对应日期和时间。
...
选择计划器中的条目或范围条，会打开文件选项模态框。

- 在 **Single date** 和 **Range** 之间切换
- 选择现有文件夹或输入自定义文件夹路径
- 编辑包含日期、范围和标题后缀的完整文件名
- 修改开始和结束日期；文件名会自动同步
- 修改条目颜色
- 修改待办/完成状态
- 修改提醒时间
...
应用更改时，Diary 会移动或重命名 Markdown 文件。转换为范围笔记时会写入 `date_start` 和 `date_end`；切回单日笔记时会删除这两个字段。如果目标文件夹已有同名文件，操作会被阻止。

桌面端可将日期条目或范围条拖到其他日期来移动。范围笔记按开始日期移动，并保持相同持续时间。如果目标路径已存在，Diary 不会移动文件。

## 键盘和可访问性

- 在聚焦的日期单元格、计划条目、范围条、节假日徽标、月度列表行、年份标签或月份标签上按 `Enter` 或 `Space` 可激活。
...
在计划视图中，选择日期或条目时按住 macOS 的 `Cmd`，或 Windows/Linux 的 `Ctrl`。
...
- 在月度网格中使用双指缩放。
- 使用重置缩放按钮恢复网格缩放级别。
- Diary 会自动让内容和菜单避开 Obsidian 的标准或浮动底部导航。使用 **移动端底部间距** 设置额外最小间距，使用 **移动端单元格宽度** 调整年度单元格。
...
| Planner note scan scope | 控制 Diary 在整个仓库中查找计划笔记，还是只在 **Planner folder** 及其子文件夹中查找。默认：`Entire vault`。 |
...
| 外部日历 | 可选 `webcal://` 或 `https://` `.ics` 订阅源，以只读叠加显示。每个订阅源都有启用状态、颜色、刷新间隔、描述开关和各视图可见性设置。 |
| 移动端底部间距 | 所有移动端计划器的最小底部间距。即使值较小，navbar 和 safe area 自动间距仍会应用。 |
| 移动端单元格宽度 | 移动端年度计划器的月份单元格宽度。`0` 使用默认值。 |
| 反馈与支持 | 打开用于反馈、错误报告和功能建议的公开 GitHub 表单。 |
...
| Key | 说明 |
| --- | --- |
| `color` | 条目颜色。可使用任何有效 CSS 颜色，例如 `#22c55e`、`red`、`rgb(34, 197, 94)`。 |
| `todo` | 为 `true` 时显示为待办条目。 |
| `completed` | 当 `todo: true` 时显示完成状态。 |
| `start_time` | 可选开始时间，格式为 `HH:mm`。在范围笔记中，它是 `date_start` 的时间边界；两个时间须同时填写或同时留空。 |
| `end_time` | 可选结束时间，格式为 `HH:mm`。在范围笔记中，它是 `date_end` 的时间边界；不会触发提醒。 |
...
| `diary_external_calendar` | 从外部事件创建 Markdown 笔记时保存的外部日历订阅源 ID。 |
...
Diary 默认扫描整个仓库中的 Markdown 文件，并显示符合以下规则的笔记。在设置中，你可以把扫描限制到 **Planner folder** 及其子文件夹。新建笔记默认放入配置的 **Planner folder**。
...
- 本仓库通过 GitHub Actions 从带标签的源代码构建 `main.js`、`manifest.json`、`styles.css`，并将其作为独立的发布资产上传。

## 反馈与支持

- [提交反馈](https://github.com/POBSIZ/obsidian-diary/issues/new?template=订阅源back.yml)
- [报告错误](https://github.com/POBSIZ/obsidian-diary/issues/new?template=bug_report.yml)
- [建议功能](https://github.com/POBSIZ/obsidian-diary/issues/new?template=feature_request.yml)

这些链接会打开公开的 GitHub Issue，并且需要 GitHub 帐户。请勿包含私密 Vault 内容、日历 URL、访问令牌或其他敏感信息。

## 隐私和网络

- 计划器功能只处理仓库内的本地 Markdown 文件。
- Diary 没有隐藏 telemetry 或 analytics。
- 节假日和日历叠加计算使用打包数据、浏览器提供的数据或本地保存的配置，不会为了计划渲染向外部服务发送仓库内容。
- 外部日历订阅源是可选的。Diary 只获取你添加的订阅源 URL，把事件缓存存入插件数据，并且只有在你选择 **Create Markdown note** 时才创建 Markdown 文件。
- 本地提醒只在 Obsidian 内处理，不会通过网络发送提醒数据。
...
- 如果条目不出现，请确认文件名符合 `YYYY-MM-DD` 或 `YYYY-MM-DD--YYYY-MM-DD`。
- 如果希望在自动避开的移动导航上方留出更多空间，请增加 **移动端底部间距**。
