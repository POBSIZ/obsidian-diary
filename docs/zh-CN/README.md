...
Diary 是一个 Obsidian 社区插件，可以把 vault 中的 Markdown 文件显示为按日期组织的计划视图。你可以在年度总览、月度网格和月度列表之间切换，同时管理单日笔记、范围笔记、月度和年度计划笔记、节假日、一个日历叠加标签、可选的外部日历、待办状态和本地提醒。
...
以下截图来自一个带有示例计划笔记的全新演示 vault。移动端截图是在启用插件移动端渲染路径后拍摄的。右侧边栏计划器使用同一个月度网格的紧凑模式，因此完整月度网格截图仍是主要视觉参考。
...
- **重复事件**：按每日、每月或每年重复笔记，可选择公历或备用日历作为基准。Diary 只会创建当前可见计划范围内的发生项。
...
- 启用 **Repeat** 后，选择每日、每月或每年，以及日历基准。发生项会按当前可见年份或月份延迟创建；Diary 只更新同一 `recurrence_id` 系列文件，并跳过无关的已有笔记。
...
| `recurrence_rule` | 简单频率规则：`FREQ=DAILY`、`FREQ=MONTHLY` 或 `FREQ=YEARLY`。 |
...
重复发生项生成是幂等的。如果目标文件已经属于同一 `recurrence_id`，Diary 会刷新其系列 metadata；如果该路径是普通笔记或另一系列，Diary 会保持不变。
