---
version: "alpha"
lastReviewedPluginVersion: "1.4.0"
...
이 문서는 `1.4.0` 기준의 플래너 UI, 오른쪽 사이드바 플래너, 월간 목록 필터, 반복 이벤트 표시, built-in/custom 캘린더 오버레이, 외부 캘린더 오버레이, 키보드 접근성 상태를 함께 설명한다.
...
`1.4.0`에서는 설정의 **Calendar overlay**에서 built-in 보조 역법 또는 custom calendar profile 하나를 선택할 수 있다.
...
`1.4.0`에서는 **External calendars** 설정에서 `webcal://` 또는 `https://` `.ics` feed를 추가하고,
수동 또는 주기 새로고침으로 가져온 이벤트를 연간/월간 그리드/월간 목록/사이드바/일자 요약에 표시한다.
