---
version: "alpha"
lastReviewedPluginVersion: "1.15.0"
...
이 문서는 `1.15.0` 기준의 연간/월간/목록/일별/3일 플래너 UI, 오른쪽 사이드바 플래너, 공통 헤더와 기간 이동, 반복 이벤트 표시, built-in/custom 캘린더 오버레이, 외부 캘린더 오버레이, 파일 옵션 모달, 다국어 UI, 키보드 접근성 상태를 함께 설명한다.
...
런타임 UI의 기준은 `styles.css`의 Obsidian 연동 변수와 planner 토큰, 그리고
`src/ui/components.ts`·`src/views/planner-components.ts`의 공통 primitive다.
이 문서는 해당 구현의 의도와 적용 규칙을 설명하며, 토큰이나 컴포넌트 동작이 바뀌면
같은 변경 단위에서 함께 갱신한다.
...
모바일 플래너의 스크롤 끝과 compact view switcher는 Obsidian의 일반/부동 하단
내비게이션 높이, navbar bottom offset, safe-area inset을 공통 clearance로 계산해 그 위에
배치한다. **Mobile bottom padding** 설정은 이 자동 clearance를 끄는 값이 아니라 사용자가
요구하는 최소 여백이며, 실제 하단 여백은 자동 clearance와 설정값 중 큰 값을 사용한다.
...
datetime 구간이다. 두 시간이 모두 없으면 종일 영역에서 날짜 열을 가로지르는 lane bar로
표시하고, 두 시간이 모두 있으면 첫날 시작 시각부터 마지막 날 종료 시각까지 날짜별 slice로
시간축에 표시한다. 종일 기간을 시간축으로 끌어 시간을 지정할 수 있고, 시간 지정 기간의
첫날 위쪽 경계와 마지막 날 아래쪽 경계는 날짜 열을 넘겨 조절할 수 있다. 같은 기간의
날짜별 slice와 종일 bar는 hover/focus 강조를 공유한다.

월간 range bar는 실제 시작일뿐 아니라 새 주와 새 달의 첫 표시 구간에도 제목을 반복한다.
연간 플래너는 매월 첫 표시 구간에 제목 칩을 다시 배치해 월·연도 경계를 넘은 기간도
무슨 일정인지 확인할 수 있게 한다.
