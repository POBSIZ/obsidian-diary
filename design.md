...
- `--planner-modal-action-bottom-padding`: 모바일 모달 작업 영역의 최소 하단 여백

모바일에서는 터치 영역을 확보하기 위해 높이와 안쪽 여백을 넓힌다. 구성 요소의
역할과 토큰의 의미는 데스크톱과 동일하게 유지한다.

모바일의 독립 버튼과 펼침 트리거는 `--planner-touch-target-min`(44px)을
최소 터치 높이로 사용하고, 모달의 주요 작업 버튼은
`--planner-touch-target-comfortable`(48px)을 사용한다. 달력 셀 내부의 파일 칩과
시간축 일정은 셀 또는 일정 표면 전체가 터치 영역이므로 이 높이를 강제로 적용하지
않는다. 두 개 이상의 작업 버튼은 2열을 기본으로 배치하고 360px 이하에서는
1열로 전환한다. 3개 작업이 있는 모달은 주요 작업을 첫 줄 전체 너비로 표시한다.
모든 모달은 모바일 화면의 좌우 안전 영역과 동적 뷰포트 높이 안에 제한하며,
본문만 스크롤하고 작업 영역은 별도 행으로 유지한다. 입력 필드와 닫기 버튼은
최소 44px, 주요 작업은 48px 터치 높이를 사용한다. 긴 상세 정보는 모바일에서
레이블과 값을 한 열로 재배치하고, 입력 포커스 시 마지막 필드까지 스크롤할 여유를
둔다. 작업 영역 아래에는 공통 최소 여백을 두고 기기의 하단 안전 영역이 더 크면
안전 영역 값을 우선한다.
...
- `planner-mobile-control-system`: 헤더, 보기 선택기, 요약 시트, 플랜 노트,
  모달, 캘린더 설정에 공통 44px/48px 터치 높이와 2열/1열 배치를 적용한다.
